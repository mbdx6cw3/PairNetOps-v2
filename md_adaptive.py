...
def simulate(simulation, system, force_field, md_params, gro, top, ml_force, output_dir):
    ...
    background_charge = True
    if background_charge:
        shift_dist = np.zeros(3,dtype=float)
        f9 = open(f"./{output_dir}/background_charges.txt", "w")
    ...
    for i in range(max_steps):
        ...
        if (i % print_data) == 0:
            wrap_coords = simulation.context.getState(getPositions=True,
                enforcePeriodicBox=True).getPositions(asNumpy=True).value_in_unit(angstrom)
            for j in range(3):
                shift_dist[j] = wrap_coords[0][j] - side_length / 2
            coords = wrap_coords[:] - shift_dist
            state = simulation.context.getState(getEnergy=True)
            ...
            # make sure all charges are in periodic box and the solute is in the centre of it.
            if background_charge:
                background_charges = np.column_stack((coords, charges))
            ...
            if not md_params["adaptive_sampling"]:
                ...
                if background_charge:
                    np.savetxt(f9, background_charges[ligand_n_atom:])
                if force_field == "pair_net":
